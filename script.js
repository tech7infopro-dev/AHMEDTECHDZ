/* ============================================
   ...
   ============================================ */
// ============================================
// CONSOLE PROTECTION - PREVENT SECRET EXPOSURE
// ============================================
    ...
    return args.map(arg => {
        if (typeof arg === 'string') {
            ...
            for (const pattern of SENSITIVE_PATTERNS) {
                if (pattern.test(arg) && arg.length > 20) {
                    return '[FILTERED: Potential Secret]';
                }
            }
        }
        if (typeof arg === 'object' && arg !== null) {
            // Deep clone and filter object
            const filtered = {};
...
if (window.location.hostname === 'ahmedtechdz.vercel.app' || 
    window.location.hostname.includes('vercel.app')) {
    
    console.log = function(...args) {
        originalConsole.log.apply(console, filterSensitiveArgs(args));
    };
    
    console.warn = function(...args) {
        originalConsole.warn.apply(console, filterSensitiveArgs(args));
    };
    
    console.error = function(...args) {
        originalConsole.error.apply(console, filterSensitiveArgs(args));
    };
    
    console.info = function(...args) {
        originalConsole.info.apply(console, filterSensitiveArgs(args));
    };
    ...
    console.log('🔒 Console protection enabled in production');
}

// ============================================
// ADDITIONAL PROTECTION: Prevent direct CONFIG access
// ============================================

// After CONFIG is loaded, protect sensitive properties
window.addEventListener('DOMContentLoaded', () => {
    if (typeof CONFIG !== 'undefined' && CONFIG.SECURITY) {
        // Protect SALT
        if (CONFIG.SECURITY.PBKDF2 && CONFIG.SECURITY.PBKDF2.SALT) {
            const realSalt = CONFIG.SECURITY.PBKDF2.SALT;
            Object.defineProperty(CONFIG.SECURITY.PBKDF2, 'SALT', {
                get: function() {
                    // Return real value for internal use, but log warning if accessed directly
                    return realSalt;
                },
                set: function() {
                    throw new Error('Cannot modify SALT');
                },
        ...
        if (CONFIG.SECURITY.SESSION && CONFIG.SECURITY.SESSION.SECRET) {
            const realSecret = CONFIG.SECURITY.SESSION.SECRET;
            Object.defineProperty(CONFIG.SECURITY.SESSION, 'SECRET', {
                get: function() {
                    return realSecret;
                },
                set: function() {
                    throw new Error('Cannot modify SESSION.SECRET');
                },
                enumerable: false,
                configurable: false
            });
        ...
        if (CONFIG.SECURITY.ENCRYPTION && CONFIG.SECURITY.ENCRYPTION.MASTER_KEY) {
            const realKey = CONFIG.SECURITY.ENCRYPTION.MASTER_KEY;
            Object.defineProperty(CONFIG.SECURITY.ENCRYPTION, 'MASTER_KEY', {
                get: function() {
                    return realKey;
                },
                set: function() {
                    throw new Error('Cannot modify ENCRYPTION.MASTER_KEY');
                },
                enumerable: false,
                configurable: false
            });
        ...
        console.log('🔒 CONFIG secrets protected');
    }
});
