...
class FirebaseManager {
    ...
    async loadInitialDataFromServer() {
        ...
        const collections = [
            ...
            { name: CONFIG.FIREBASE.COLLECTIONS.TUTORIAL_VIDEOS, key: 'videos' }
        ];
    ...
    updateLocalData(type, remoteData) {
        try {
            switch(type) {
                ...
                case 'videos': this.updateLocalVideos(remoteData); break;
            }
        } catch (error) {
    ...
    updateLocalVideos(remoteVideos) {
        if (typeof videoManager === 'undefined') return;

        try {
            const processedVideos = remoteVideos.map(d => ({
                id: parseInt(d.id) || d.id,
                title: d.title,
                url: d.url,
                platform: d.platform,
            ...
            const hasChanges = JSON.stringify(videoManager.videos) !== JSON.stringify(processedVideos);

            if (hasChanges) {
                console.log('[Firebase] Updating videos:', processedVideos.length);
                videoManager.videos = processedVideos;
                
                // Update nextVideoId
                const numericIds = processedVideos.map(v => parseInt(v.id)).filter(id => !isNaN(id));
                const maxId = Math.max(...numericIds, 0);
                if (maxId >= videoManager.nextVideoId) {
                    videoManager.nextVideoId = maxId + 1;
                    localStorage.setItem(CONFIG.STORAGE_KEYS.NEXT_VIDEO_ID, videoManager.nextVideoId);
                }
                
                securityManager.secureStore(CONFIG.STORAGE_KEYS.TUTORIAL_VIDEOS, processedVideos);

                if (document.getElementById('tutorial-videos-section')?.style.display !== 'none') {
                    updateTutorialVideosGrid();
                }
            }
        } catch (error) {
            console.error('[Firebase] Error updating videos:', error);
        }
    }
    ...
    setupRealtimeListeners() {
        ...
        this.unsubscribeVideos = setupListener(
            CONFIG.FIREBASE.COLLECTIONS.TUTORIAL_VIDEOS, 
            'Videos', 
            (snapshot) => this.handleCollectionChange('videos', snapshot)
        );
...
function hideAllSections() {
    ...
    document.getElementById('tutorial-videos-section').style.display = 'none';
...
document.getElementById('tutorial-videos-card').addEventListener('click', function() {
    showTutorialVideosSection();
});
...
class VideoManager {
    ...
    loadVideos() {
        try {
            const saved = securityManager.secureRetrieve(CONFIG.STORAGE_KEYS.TUTORIAL_VIDEOS);
            if (saved && Array.isArray(saved)) {
                this.videos = saved;
                const numericIds = this.videos.map(v => parseInt(v.id)).filter(id => !isNaN(id));
                this.nextVideoId = Math.max(...numericIds, 0) + 1;
            } else {
                this.videos = [];
                this.nextVideoId = 1;
            }
        } catch (e) {
            console.error('[VideoManager] Error loading videos:', e);
            this.videos = [];
            this.nextVideoId = 1;
        }
    }
    ...
    saveVideos() {
        try {
            securityManager.secureStore(CONFIG.STORAGE_KEYS.TUTORIAL_VIDEOS, this.videos);
            localStorage.setItem(CONFIG.STORAGE_KEYS.NEXT_VIDEO_ID, this.nextVideoId);
        } catch (e) {
            console.error('[VideoManager] Error saving videos:', e);
        }
    }
        ...
        try {
            ...
            if (platform === 'shorts') {
                ...
                if (url.includes('youtube.com/shorts/')) {
                    ...
                    videoId = url.split('youtu.be/shorts/')[1]?.split('?')[0];
                }
            ...
            if (platform === 'tiktok') {
                // TikTok doesn't support direct embedding easily
                // Return the original URL for now
                return url;
            }
        } catch (e) {
            console.error('[VideoManager] Error parsing URL:', e);
        }
    ...
    addVideo(title, url) {
        const validation = securityManager.validateInput(title, 'string');
        if (!validation.valid) {
        ...
        const platform = this.getVideoPlatform(url);
        if (platform === 'unknown') {
            return { success: false, error: 'URL must be from YouTube, TikTok, or YouTube Shorts' };
        }

        const video = {
            id: this.nextVideoId++,
            title: securityManager.sanitizeInput(title.trim()),
            url: urlValidation.sanitized,
            platform: platform,
            embedUrl: this.getVideoEmbedUrl(url),
            created: new Date().toISOString(),
            createdBy: (userManager && userManager.currentUser) ? userManager.currentUser.email : 'unknown'
        };
        ...
        // Sync with Firebase
        if (firebaseManager.shouldSync()) {
            firebaseManager.syncVideo(video);
        } else {
            firebaseManager.queueOperation('syncVideo', video);
        }
    ...
    updateVideo(id, title, url) {
        ...
        // Sync with Firebase
        if (firebaseManager.shouldSync()) {
            firebaseManager.syncVideo(this.videos[videoIndex]);
        } else {
            firebaseManager.queueOperation('syncVideo', this.videos[videoIndex]);
        }

        return { success: true, video: this.videos[videoIndex] };
    }
    ...
    deleteVideo(id) {
        ...
        const videoId = this.videos[videoIndex].id;
        this.videos.splice(videoIndex, 1);
        this.saveVideos();

        // Sync with Firebase
        if (firebaseManager.shouldSync()) {
            firebaseManager.deleteVideo(videoId);
        } else {
            firebaseManager.queueOperation('deleteVideo', { videoId });
        }
    ...
    canManageVideos() {
        const user = userManager ? userManager.currentUser : null;
        if (!user) return false;
        const perms = CONFIG.TUTORIAL_VIDEOS_PERMISSIONS[user.role];
        return perms && perms.canAdd;
    }
...
// ============================================
// TUTORIAL VIDEOS UI FUNCTIONS
// ============================================

let editingVideoId = null;

function showTutorialVideosSection() {
    const user = userManager ? userManager.currentUser : null;
    if (!user) {
    ...
    hideAllSections();
    document.getElementById('tutorial-videos-section').style.display = 'block';
    document.getElementById('back-to-dashboard-btn').style.display = 'flex';
    document.getElementById('header-logout-btn').style.display = 'flex';
    
    updateTutorialVideosUI();
    updateTutorialVideosGrid();
}

function updateTutorialVideosUI() {
    const addBtn = document.getElementById('add-video-btn');
    const formContainer = document.getElementById('video-form-container');
    
    if (videoManager.canManageVideos()) {
        addBtn.style.display = 'flex';
    } else {
        addBtn.style.display = 'none';
...
function updateTutorialVideosGrid() {
    const grid = document.getElementById('tutorial-videos-grid');
    ...
    if (videos.length === 0) {
        grid.innerHTML = `
            <div class="empty-state">
                ...
                ${videoManager.canManageVideos() ? '<p>Click "Add Video" to add your first tutorial</p>' : ''}
            </div>
        `;
        return;
    }
    ...
    grid.innerHTML = videos.map(video => {
        const platformIcon = {
            ...
            'shorts': '<i class="fab fa-youtube"></i>',
            'tiktok': '<i class="fab fa-tiktok"></i>',
            'unknown': '<i class="fas fa-video"></i>'
        }[video.platform];
        
        const platformClass = video.platform;
        const date = new Date(video.created).toLocaleDateString();
        
        const canManage = videoManager.canManageVideos();
        const canDelete = videoManager.canDeleteVideos();
        
        let actionsHtml = `
            <button class="video-action-btn copy" onclick="copyVideoUrl(${video.id})" title="Copy URL">
                <i class="fas fa-copy"></i>
            </button>
        `;
        ...
        if (canManage) {
            actionsHtml += `
                <button class="video-action-btn edit" onclick="editVideo(${video.id})" title="Edit">
                    <i class="fas fa-edit"></i>
                </button>
            `;
        }
        ...
        if (canDelete) {
            actionsHtml += `
                <button class="video-action-btn delete" onclick="confirmDeleteVideo(${video.id})" title="Delete">
                    <i class="fas fa-trash"></i>
                </button>
            `;
        }
        ...
        return `
            <div class="video-card" data-video-id="${video.id}">
                ...
                <div class="video-player-container">
                    <span class="video-platform-badge ${platformClass}">${platformIcon} ${video.platform}</span>
                    ${getVideoPlayerHtml(video)}
                </div>
...
function getVideoPlayerHtml(video) {
    if (!video.embedUrl) {
        return `<div style="display: flex; align-items: center; justify-content: center; height: 100%; color: rgba(255,255,255,0.5);">
            <i class="fas fa-exclamation-circle"></i> Invalid video URL
        </div>`;
    }
    
    if (video.platform === 'tiktok') {
        // For TikTok, show a link button since embedding is complex
        return `
            <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; background: linear-gradient(45deg, #000000, #1a1a1a);">
                <i class="fab fa-tiktok" style="font-size: 40px; color: white; margin-bottom: 10px;"></i>
                <a href="${video.url}" target="_blank" style="color: white; text-decoration: none; padding: 10px 20px; background: rgba(255,255,255,0.2); border-radius: 20px; font-size: 13px;">
                    <i class="fas fa-external-link-alt"></i> Open on TikTok
                </a>
            </div>
        `;
    }
    
    // YouTube and Shorts use iframe embedding
    return `<iframe src="${video.embedUrl}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
}

function showVideoForm() {
    const formContainer = document.getElementById('video-form-container');
    const formTitle = document.getElementById('video-form-title');
    const saveBtnText = document.getElementById('save-video-btn-text');
...
function saveVideo() {
    ...
    if (editingVideoId) {
        result = videoManager.updateVideo(editingVideoId, title, url);
        if (result.success) {
            showNotification('Video updated successfully!', 'success');
        }
    } else {
        result = videoManager.addVideo(title, url);
        if (result.success) {
    ...
    if (result.success) {
        ...
        showNotification(result.error || 'Failed to save video', 'error');
    }
}

function editVideo(id) {
    const video = videoManager.getVideoById(id);
    if (!video) {
    ...
    document.getElementById('save-video-btn-text').textContent = 'Update Video';
...
function confirmDeleteVideo(id) {
    ...
    deleteInfo.innerHTML = `
        <div class="video-info-row">
            <span class="video-info-label">ID:</span>
            <span class="video-info-value">#${video.id}</span>
        </div>
        <div class="video-info-row">
            <span class="video-info-label">Title:</span>
            <span class="video-info-value">${escapeHtml(video.title)}</span>
        </div>
        <div class="video-info-row">
            <span class="video-info-label">Platform:</span>
            <span class="video-info-value">${video.platform}</span>
        </div>
    `;
    
    document.getElementById('deleteVideoModal').dataset.videoId = id;
    document.getElementById('deleteVideoModal').classList.add('active');
}

function deleteVideo() {
    const modal = document.getElementById('deleteVideoModal');
    const id = modal.dataset.videoId;
    
    if (!id) return;
    
    const result = videoManager.deleteVideo(id);
...
function closeDeleteVideoModal() {
    ...
    document.getElementById('deleteVideoModal').dataset.videoId = '';
}
...
// ============================================
// FIREBASE SYNC FOR VIDEOS
// ============================================

// Add sync methods to FirebaseManager prototype
FirebaseManager.prototype.syncVideo = async function(videoData) {
    await this.ensureInitialized();
    if (!this.shouldSync()) {
        this.queueOperation('syncVideo', videoData);
        return { success: false, offline: true };
    }

    try {
        const videoRef = this.db.collection(CONFIG.FIREBASE.COLLECTIONS.TUTORIAL_VIDEOS)
                                .doc(videoData.id.toString());
        await videoRef.set({
            ...videoData,
            lastSync: firebase.firestore.FieldValue.serverTimestamp()
        }, { merge: true });
...
FirebaseManager.prototype.deleteVideo = async function(videoId) {
    await this.ensureInitialized();
    if (!this.shouldSync()) {
        this.queueOperation('deleteVideo', { videoId });
        return { success: false, offline: true };
    }

    try {
        await this.db.collection(CONFIG.FIREBASE.COLLECTIONS.TUTORIAL_VIDEOS)
                    .doc(videoId.toString()).delete();
        return { success: true };
    } catch (error) {
        this.queueOperation('deleteVideo', { videoId });
        return { success: false, error: error.message };
...
// Update processOfflineQueue to handle video operations
const originalProcessOfflineQueue = FirebaseManager.prototype.processOfflineQueue;
FirebaseManager.prototype.processOfflineQueue = async function() {
    if (this.offlineQueue.length === 0) return;

    const videoOperations = [];
    const otherOperations = [];

    // Separate video operations from other operations
    for (const item of this.offlineQueue) {
        if (item.operation === 'syncVideo' || item.operation === 'deleteVideo') {
            videoOperations.push(item);
        } else {
    ...
    const failedVideoOperations = [];
    for (const item of videoOperations) {
        try {
            let result;
            switch (item.operation) {
                case 'syncVideo':
                    result = await this.syncVideo(item.data);
                    break;
                case 'deleteVideo':
                    result = await this.deleteVideo(item.data.videoId);
                    break;
            }
            ...
            if (!result || !result.success) {
                item.retryCount++;
                if (item.retryCount < 3) {
                    failedVideoOperations.push(item);
                }
            }
        } catch (error) {
            item.retryCount++;
    ...
    // Set offline queue to only other operations for original method
    this.offlineQueue = otherOperations;

    // Call original method for other operations
    await originalProcessOfflineQueue.call(this);

    // Restore failed video operations back to queue
    this.offlineQueue = [...this.offlineQueue, ...failedVideoOperations];
};

// ============================================
// EVENT LISTENERS FOR TUTORIAL VIDEOS
// ============================================

document.addEventListener('DOMContentLoaded', function() {
    // Dashboard card click
    const tutorialVideosCard = document.getElementById('tutorial-videos-card');
    if (tutorialVideosCard) {
        tutorialVideosCard.addEventListener('click', function() {
            const user = userManager ? userManager.currentUser : null;
            if (!user) {
                showNotification('Please login first', 'error');
                return;
    ...
    if (cancelVideoBtn) {
        cancelVideoBtn.addEventListener('click', hideVideoForm);
    }
    ...
    const deleteVideoModalClose = document.getElementById('deleteVideoModalClose');
    const deleteVideoModalCancel = document.getElementById('deleteVideoModalCancel');
    const deleteVideoModalConfirm = document.getElementById('deleteVideoModalConfirm');
    
    if (deleteVideoModalClose) {
        deleteVideoModalClose.addEventListener('click', closeDeleteVideoModal);
    }
    if (deleteVideoModalCancel) {
        deleteVideoModalCancel.addEventListener('click', closeDeleteVideoModal);
    }
    if (deleteVideoModalConfirm) {
        deleteVideoModalConfirm.addEventListener('click', deleteVideo);
    }
    
    // Close modal on backdrop click
    const deleteVideoModal = document.getElementById('deleteVideoModal');
    if (deleteVideoModal) {
        deleteVideoModal.addEventListener('click', function(e) {
            if (e.target === this) {
                closeDeleteVideoModal();
            }
        });
...
console.log('%c[VideoManager] Tutorial Videos module loaded successfully!', 'color: #9b59b6; font-weight: bold;');
