...
// Reads from Environment Variables (Vercel) or uses safe defaults
// ============================================

// ============================================
// SECURE ENVIRONMENT LOADER - NO DEFAULTS FOR SECRETS
// ============================================
const getEnv = (key, defaultValue = null, isSecret = false) => {
    // Check for meta tags (injected by build.js from Vercel)
    ...
    // For secrets: NEVER return default - return null if not found
    if (isSecret) {
        return null;
    }
...
// ✅ CRITICAL: No default password - must come from Vercel
const DEFAULT_OWNER_PASSWORD = getEnv('NEXT_PUBLIC_DEFAULT_OWNER_PASSWORD', null, true);

// Warning if password not set (only in development)
if (!DEFAULT_OWNER_PASSWORD && window.location.hostname === 'localhost') {
    console.warn('⚠️ NEXT_PUBLIC_DEFAULT_OWNER_PASSWORD not set in Vercel env vars!');
}
...
const CONFIG = {
    // ============================================
    // FIREBASE CONFIGURATION (Public - OK to show)
    // ============================================
    FIREBASE: {
    ...
    // All secrets loaded from Vercel, NO defaults
    // ============================================
    SECURITY: {
        PBKDF2: {
            ...
            AUTO_UPGRADE: true,
            // ✅ NO DEFAULT - must come from Vercel
    ...
    // ============================================
    // RESOURCE PERMISSIONS (unchanged)
    // ============================================
    MAC_PERMISSIONS: {
...
// ============================================
// PROTECTION: Override toString to prevent accidental logging
// ============================================
